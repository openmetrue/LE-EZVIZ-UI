...
func handleLogs(w http.ResponseWriter, r *http.Request) {
	...
	logBtn := func(val, key string) string {
		on := logOn == (val == "1")
		cls, dis := "", ""
		if on {
			cls = ` class="on"`
		...
		return `<form method="post"><input type="hidden" name="action" value="bridgelog"><input type="hidden" name="on" value="` + val + `"><input type="hidden" name="f" value="` + esc(cur) + `"><button type="submit"` + cls + dis + `>` + esc(T(lang, key)) + `</button></form>`
	}
	msg := ""
	...
	render(w, r, T(lang, "logs.title"), tabs(r, "logs")+
		...
		`<div class="seg">`+logBtn("1", "logs.on")+logBtn("0", "logs.off")+`</div>`+
		msg+
